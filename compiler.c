...
__inline__ void m68k_do_jsr(CPTR dest)
{
...
void m68k_do_bsr(LONG offset)
{
    m68k_do_jsr(m68k_getpc() + offset);
}
...
static __inline__ void assemble_ulong_68k(ULONG a)
{
    assemble(a >> 24);
    assemble(a >> 16);
    assemble(a >> 8);
			      ...
			      int pidmult)
{
	...
	break;
     ...
     case Ad16:
	eai->addr_const_off = (WORD)do_get_mem_word((UWORD *)p);
	(*pcpp) += 2; p += 2;
	...
	break;
     ...
     case Ad8r:
	dp = (WORD)do_get_mem_word((UWORD *)p);
	r = (dp & 0x7000) >> 12;
	(*pcpp) += 2; p += 2;	
	...
	if (dp & 0x800) {
	    ...
	    } else if ((LONG)eai->addr_const_off >= -128 && (LONG)eai->addr_const_off <= 127) {
		assemble(0x8D); 
	...
	break;
     ...
     case PC16:
	eai->addr_const_off = pca + (WORD)do_get_mem_word((UWORD *)p);
	eai->address_reg = -2;
	(*pcpp) += 2; p += 2;
	break;
	...
	break;
     ...
     case absl:
	eai->addr_const_off = (LONG)do_get_mem_long((ULONG *)p);
	eai->address_reg = -2;
	(*pcpp) += 4; p += 4;
	break;
	...
	break;
     ...
     case imm1:
	imm1_const:
	eai->data_const_off = (WORD)do_get_mem_word((UWORD *)p);
	eai->data_reg = -2;
	(*pcpp) += 2; p += 2;
	break;
     ...
     case imm2:
	imm2_const:
	eai->data_const_off = (LONG)do_get_mem_long((ULONG *)p);
	eai->data_reg = -2;
	(*pcpp) += 4; p += 4;
	break;
...
static __inline__ void rmop_finalize(struct ea_info *eai) 
{
	...
	if (eai->address_reg == -2)
	    /* Constant address */
	    assemble_long(address_space + (LONG)eai->addr_const_off);
			    ...
			    int eaino_s, int eaino_d)
{
	...
	if (map->dreg_map[eaid->reg] < 0) {
		...
		if (eais->mode == Dreg) {
		    compile_move_reg_from_mem_regoffs(destr, -2, (ULONG)(regs.regs + eais->reg),
						      eais->size);
		    map->x86_byteorder[destr] = BO_NORMAL;
		} else if (eais->mode == Areg) {
		    compile_move_reg_from_mem_regoffs(destr, -2, (ULONG)(regs.regs + 8 + eais->reg),
						      eais->size);
		    map->x86_byteorder[destr] = BO_NORMAL;
		} else {
		    /* Move mem to reg */
		    ...
		    compile_move_reg_from_mem_regoffs(destr, eais->address_reg,
						      (ULONG)(eais->addr_const_off + address_space),
						      eais->size);
	...
	compile_move_reg_to_mem_regoffs(eaid->address_reg,
					(ULONG)(eaid->addr_const_off + address_space),
				  ...
				  int user_live_at_end, int user_ccval)
{
	...
	if (cc_reg == -2) {
	    ...
	    compile_move_reg_to_mem_regoffs(-2, (ULONG)&regflags, tmpr, sz_long);
	}
	...
	if (status == CC_AFTER_ROX)
	    compile_move_reg_to_mem_regoffs(-2, 4 + (ULONG)&regflags, tmpr, sz_long);
    } else if (status != 0) {
	...
	if (status == CC_C_FROM_86C && (live_at_end & CC68K_C) != 0) 
		...
		if ((status & CC_X_FROM_86C) != 0 && live_at_end == CC68K_X) {
		    ...
		    assemble(0x05); assemble_long(4 + (ULONG)&regflags);
		} else {
		    int tmpr = get_free_x86_register(map, ALL_X86_REGS);
		    /* pushfl; popl tmpr; movl tempr, regflags */
		    assemble(0x9C); assemble(0x58+tmpr);
			...
			compile_move_reg_to_mem_regoffs(-2, 4 + (ULONG)&regflags, tmpr, sz_word);
		    }
		}
	    }
	}
...
static void handle_rotshi(struct register_mapping *map, int iip, 
			  UBYTE *realpc, CPTR current_addr, struct pid_undo *pud)
{
    struct ea_info eai;
	...
	remove_x86r_from_cache(map, r_EAX, 1);
	lock_reg(map, r_EAX, 2);
	compile_move_reg_from_mem_regoffs(r_AH, -2, 4 + (ULONG)&regflags, sz_byte);
    }
				   ...
				   struct pid_undo *pud)
{
	...
	remove_x86r_from_cache(map, r_EAX, 1);
	lock_reg(map, r_EAX, 2);
	compile_move_reg_from_mem_regoffs(r_AH, -2, 4 + (ULONG)&regflags,
					  sz_byte);
    }
...
static int m68k_compile_block(struct hash_block *hb)
{
	...
	if (insn_info[iip].stop_translation == 1) {
	 ...
	 case i_BSR:
	    ...
	    {
		char *tmp1, *tmp2, *tmp3;
		CPTR dest = insn_info[iip].address + 2 + (LONG)eainfo[0].data_const_off;
		struct hash_entry *tmph;
	 ...
	 case i_SUBX:
	    ...
	    assemble_ulong(4 + (ULONG)&regflags); assemble(0);
	 ...
	 case i_LINK:
	    ...
	    eainfo[4].data_const_off += (WORD)eainfo[1].data_const_off;
	 ...
	 case i_MVMEL:
		...
		compile_lea_reg_with_offset(scratch1, -2, (ULONG)regs.regs);
