...
static void generate_func(void)
{
	...
	printf("{\n");
	printf("int i,j;\n"); 
	printf("ULONG totald = 0;\n");
	printf("if (blits_32bit_enabled && b->hblitsize > 1");
	...
	printf("} else {\n");
	if (a_is_on) printf("ULONG preva = 0;\n");
	if (b_is_on) printf("ULONG prevb = 0, srcb = b->bltbhold;\n");
	if (c_is_on) printf("ULONG srcc = b->bltcdat;\n");
	printf("for (j = 0; j < b->vblitsize; j++) {\n");
	printf("\tfor (i = 0; i < b->hblitsize; i++) {\t\tULONG dstd, bltadat, srca;\n\n");
	if (c_is_on) printf("\t\tif (ptc) { srcc = do_get_mem_word((UWORD *)ptc); ptc += 2; }\n");
	if (b_is_on) printf("\t\tif (ptb) {\n\t\t\tULONG bltbdat = do_get_mem_word((UWORD *)ptb); ptb += 2;\n");
	if (b_is_on) printf("\t\t\tsrcb = (((ULONG)prevb << 16) | bltbdat) >> b->blitbshift;\n");
	if (b_is_on) printf("\t\t\tprevb = bltbdat;\n\t\t}\n");
	if (a_is_on) printf("\t\tif (pta) { bltadat = do_get_mem_word((UWORD *)pta); pta += 2; } else { bltadat = blt_info.bltadat; } \n");
	if (a_is_on) printf("\t\tbltadat &= blit_masktable[i];\n");
        if (a_is_on) printf("\t\tsrca = (((ULONG)preva << 16) | bltadat) >> b->blitashift;\n");
	if (a_is_on) printf("\t\tpreva = bltadat;\n");
	printf("\t\tdstd = (%s) & 0xFFFF;\n", blitops[blttbl[i]].s);
	printf("\t\ttotald |= dstd;\n");
	printf("\t\tif (ptd) { do_put_mem_word((UWORD *)ptd, dstd); ptd += 2; }\n");
	printf("\t}\n");
	if (a_is_on) printf("\tif (pta) pta += b->bltamod;\n");
	if (b_is_on) printf("\tif (ptb) ptb += b->bltbmod;\n");
	if (c_is_on) printf("\tif (ptc) ptc += b->bltcmod;\n");
	...
	printf("}\n");
	printf("if (totald == 0) b->blitzero = 1;\n");
	printf("}\n");	
	...
	if (a_is_on) printf("\t\tif (pta) { bltadat = do_get_mem_word((UWORD *)pta); pta -= 2; } else { bltadat = blt_info.bltadat; } \n");
	if (a_is_on) printf("\t\tbltadat &= blit_masktable[i];\n");
        if (a_is_on) printf("\t\tsrca = (((ULONG)bltadat << 16) | preva) >> b->blitdownashift;\n");
	if (a_is_on) printf("\t\tpreva = bltadat;\n");
	printf("\t\tdstd = (%s) & 0xFFFF;\n", blitops[blttbl[i]].s);
	printf("\t\ttotald |= dstd;\n");
	printf("\t\tif (ptd) { do_put_mem_word((UWORD *)ptd, dstd); ptd -= 2; }\n");
