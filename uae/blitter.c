...
ULONG blit_masktable[BLITTER_MAX_WORDS];
static UWORD blit_trashtable[BLITTER_MAX_WORDS];
...
static void blitter_dofast(void) 
{
	...
	ULONG blitbhold = blt_info.bltbhold;
	ULONG preva = 0, prevb = 0;

	/*if (!blitfill) fprintf(stderr, "minterm %x not present\n",mt); */
        for (j = 0; j < blt_info.vblitsize; j++) {
	    blitfc = !!(bltcon1 & 0x4);
	    for (i = 0; i < blt_info.hblitsize; i++) {
		ULONG bltadat, blitahold;
	        if (bltadatpt) {
		    bltadat = do_get_mem_word((UWORD *)bltadatpt); bltadatpt += 2;
		} else
		    bltadat = blt_info.bltadat;
		bltadat &= blit_masktable[i];
		blitahold = (((ULONG)preva << 16) | bltadat) >> blt_info.blitashift;
		preva = bltadat;

	        if (bltbdatpt) {
		    UWORD bltbdat = do_get_mem_word((UWORD *)bltbdatpt); bltbdatpt += 2;
		    blitbhold = (((ULONG)prevb << 16) | bltbdat) >> blt_info.blitbshift;
		    prevb = bltbdat;
		}
		if (bltcdatpt) {
		    blt_info.bltcdat = do_get_mem_word((UWORD *)bltcdatpt); bltcdatpt += 2; 
		}
	        ...
	        if (bltddatpt) {
		    do_put_mem_word((UWORD *)bltddatpt, blt_info.bltddat);
		    bltddatpt += 2;
		}
	    }
...
/*	if (!blitfill) fprintf(stderr, "minterm %x not present\n",mt);*/
		...
		ULONG bltadat, blitahold;
		    ...
		    bltadat = blt_info.bltadat;
		bltadat &= blit_masktable[i];
		blitahold = (((ULONG)bltadat << 16) | preva) >> blt_info.blitdownashift;
		preva = bltadat;
	        if (bltbdatpt) {
		    UWORD bltbdat = do_get_mem_word((UWORD *)bltbdatpt); bltbdatpt -= 2;
		    blitbhold = (((ULONG)bltbdat << 16) | prevb) >> blt_info.blitdownbshift;
		    prevb = bltbdat;
		}
		if (bltcdatpt) {
		    blt_info.bltcdat = do_get_mem_word((UWORD *)bltcdatpt); bltcdatpt -= 2; 
