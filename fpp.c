...
get_fp_value(ULONG opcode, UWORD extra,double *src)
{
	...
	case 0:
		switch (size) {
		case 6: *src = (double)(BYTE)m68k_dreg(regs, reg);
			break;
		case 4: *src = (double)(WORD)m68k_dreg(regs, reg);
			break;
		case 0: *src = (double)(LONG)m68k_dreg(regs, reg);
			break;
	...
	case 4:
		...
		break;
	case 5:
		ad = m68k_areg(regs, reg) + (LONG)(WORD)nextiword();
		break;
	case 6:
